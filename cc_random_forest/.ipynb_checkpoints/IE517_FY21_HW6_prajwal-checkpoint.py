...
for i in range(1,10):
    ...
    clf = RandomForestClassifier(n_estimators=25*i, max_depth=2, random_state=0)
    skf = StratifiedKFold(n_splits=10)
    tic=time.time()
    score1[i]=cross_val_score(clf, X_train, y_train, cv=skf)
    toc[i]=time.time()-tic
    print('mean accuracy with stratified k fold for n_estimators=',str(25*i),'is:',str(np.mean(score1)))
    
plt.bar(range(1,10)*25,toc)
plt.xlabel('n_estimators')
plt.ylabel('time taken to train in seconds')
plt.title('Training time vs number of estimators')
plt.show()

plt.bar(range(1,10)*25,score1)
plt.xlabel('n_estimators')
